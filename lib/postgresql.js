 ...
 */
PostgreSQL.prototype.columnEscaped = function(model, property, expression=null) {
  if (expression && expression.constructor === Object) {
    const operator = Object.keys(expression)[0];
    if (operator === '@>' && isNested(property)) {
      const self = this;
      return property
        .split('.')
...
PostgreSQL.prototype.buildExpression = function(columnName, operator,
  ...
  switch (operator) {
    case 'equnc':
      return new ParameterizedSQL(`f_unaccent(${columnName})` + "::TEXT = f_unaccent(?)", [operatorValue]);
    case 'nequnc':
      return new ParameterizedSQL(`f_unaccent(${columnName})` + "::TEXT != f_unaccent(?)", [operatorValue]);
    case 'likeunc':
      return new ParameterizedSQL(`f_unaccent(${columnName})` + "::TEXT LIKE f_unaccent(?)", [operatorValue]);
    case 'ilikeunc':
      return new ParameterizedSQL(`f_unaccent(${columnName})` + "::TEXT ILIKE f_unaccent(?)", [operatorValue]);
    case 'nlikeunc':
      return new ParameterizedSQL(`f_unaccent(${columnName})` + "::TEXT NOT LIKE f_unaccent(?)", [operatorValue]);
    case 'nilikeunc':
      return new ParameterizedSQL(`f_unaccent(${columnName})` + "::TEXT NOT ILIKE f_unaccent(?)", [operatorValue]);
    case 'like':
    ...
    case '@>':
      return new ParameterizedSQL(columnName + " @> ?::"
        + propertyDefinition.postgresql.dataType, [operatorValue]);
    case 'contains':
...
PostgreSQL.prototype._buildWhere = function(model, where) {
  ...
  for (const key in where) {
    ...
    const columnName = self.columnEscaped(model, key, expression);
